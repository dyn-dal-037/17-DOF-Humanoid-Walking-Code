...
// NEW 7-STEP WALKING SEQUENCE
int step1[NUM_SERVOS] = {80, 80, 30, 40, 30, 50, 144, 30, 70, 70, 2, 76, 40, 168, 30, 168, 108};
int step2[NUM_SERVOS] = {75, 50, 25, 70, 30, 50, 144, 30, 70, 70, 2, 76, 40, 168, 30, 168, 108};
int step3[NUM_SERVOS] = {75, 50, 25, 65, 35, 55, 144, 15, 70, 80, 2, 76, 40, 168, 30, 168, 108};
int step4[NUM_SERVOS] = {75, 75, 25, 35, 35, 55, 144, 40, 95, 80, 2, 76, 40, 168, 30, 168, 108};
int step5[NUM_SERVOS] = {90, 75, 25, 35, 45, 65, 160, 40, 65, 90, 2, 76, 40, 168, 30, 168, 108};
int step6[NUM_SERVOS] = {85, 75, 25, 35, 35, 60, 160, 40, 65, 80, 2, 76, 40, 168, 30, 168, 108};
int step7[NUM_SERVOS] = {85, 65, 15, 40, 35, 60, 160, 40, 65, 80, 2, 76, 40, 168, 30, 168, 108};
...
int stepDelay = 600;  // Step timing
int moveSpeed = 25;   // Servo movement speed
...
void setup() {
  ...
  Serial.println(F("\n===== 17 DOF HUMANOID ROBOT - 7 STEP WALK ====="));
  initializeRobot();
  setDefaultPosition();
  ...
  Serial.println(F("✓ New 7-step walking sequence loaded"));
...
void printMenu() {
  Serial.println(F("\n===== 🤖 ROBOT CONTROL MENU ====="));
  Serial.println(F("🚶 walk / w      - Start 7-step walking"));
...
void printStatus() {
  ...
  Serial.print(F("Current Step: ")); Serial.print(currentStep + 1); Serial.println(F("/7"));
  Serial.println(F("====================\n"));
}

void startWalking() {
  Serial.println(F("🚶 Starting 7-step walking sequence"));
  ...
  if (useSmoothing) {
    smoothMoveToPosition(step1);
  } else {
    moveToPosition(step1);
  }
}
...
void executeWalkingStep() {
  if (millis() - lastStepTime >= stepDelay) {
    ...
    // Execute the 7-step sequence
    switch (currentStep) {
      case 0:
        Serial.println(F("Step 1/7"));
        if (useSmoothing) smoothMoveToPosition(step1);
        else moveToPosition(step1);
        break;
      case 1:
        Serial.println(F("Step 2/7"));
        if (useSmoothing) smoothMoveToPosition(step2);
        else moveToPosition(step2);
        break;
      case 2:
        Serial.println(F("Step 3/7"));
        if (useSmoothing) smoothMoveToPosition(step3);
        else moveToPosition(step3);
        break;
      case 3:
        Serial.println(F("Step 4/7"));
        if (useSmoothing) smoothMoveToPosition(step4);
        else moveToPosition(step4);
        break;
      case 4:
        Serial.println(F("Step 5/7"));
        if (useSmoothing) smoothMoveToPosition(step5);
        else moveToPosition(step5);
        break;
      case 5:
        Serial.println(F("Step 6/7"));
        if (useSmoothing) smoothMoveToPosition(step6);
        else moveToPosition(step6);
        break;
      case 6:
        Serial.println(F("Step 7/7"));
        if (useSmoothing) smoothMoveToPosition(step7);
        else moveToPosition(step7);
        break;
    }
    ...
    if (currentStep >= 7) {
      currentStep = 0;
      Serial.println(F("🔄 7-step cycle complete - Repeating"));
    }
  }
}
